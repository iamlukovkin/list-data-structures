#ifndef POSTFIX_EVALUATOR_TEST_H
#define POSTFIX_EVALUATOR_TEST_H
...
public:
    explicit PostfixEvaluatorTest(Stack<int>* stackImpl) : stack(stackImpl) {}

    void run(const vector<PostfixTestCase>& testCases) const {
        const PostfixEvaluator evaluator(stack);
        int passed = 0;
        const int total = static_cast<int>(testCases.size());
        string title = "--- Testing PostfixEvaluator ---";
        ranges::transform(title, title.begin(), ::toupper);
...
#endif // POSTFIX_EVALUATOR_TEST_H
